...
vector<vector<double>> Dubins::interpolate(
    double ind,
    double length,
    char m,
...
vector<vector<double>> Dubins::generate_local_course(
    double total_length,
    ...
    double step_size)
{
    ...
    for (int i = 0; i < modes.size(); i++)
    {
        ...
        vector<vector<double>> inter =
            interpolate(ind, lengths[i], modes[i], max_curvature, origin_x, origin_y, origin_yaw, p_x, p_y, p_yaw, directions);
        p_x = inter[0], p_y = inter[1], p_yaw = inter[2], directions = inter[3];
    }
    if (p_x.size() <= 1)
    {
...
Dubins::ResultDubins Dubins::dubins_path_planning_from_origin(Vector3d goal, double curvature, double step_size)
{
    ...
    {
        if (costs[i] < best_cost && costs[i] > 0)
        {
            best_cost = costs[i];
        }
    }
    ...
    vector<vector<double>> glc = generate_local_course(lengths[0] + lengths[1] + lengths[2], lengths, best_mode, curvature, step_size);
...
Dubins::ResultDubins Dubins::dubins_path_planning(Vector3d start, Vector3d goal, double curvature, double step_size)
{
    ResultDubins res;
    goal[0] -= start[0];
    goal[1] -= start[1];
    Matrix3d l_rot;
    l_rot              = AngleAxisd(start[2], Eigen::Vector3d::UnitZ());
    MatrixXd block_rot = l_rot.block(0, 0, 2, 2);
    ...
    Matrix3d rot;
    rot = AngleAxisd(-start[2], Eigen::Vector3d::UnitZ());
    assert(rd.p_x.size() == rd.p_y.size() && rd.p_y.size() == rd.p_yaw.size());
    int sz = rd.p_x.size();
    MatrixXd pxy(sz, 2);
    ...
    MatrixXd block_rot2   = rot.block(0, 0, 2, 2);
