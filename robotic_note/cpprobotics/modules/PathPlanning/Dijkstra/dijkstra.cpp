...
public:
    ...
    Node(int x_, int y_, double cost_, Node* p_node_ = NULL) : x(x_), y(y_), cost(cost_), p_node(p_node_){};
};
...
double calc_heristic(Node n1, Node n2, double w = 1.0)
{
...
std::pair<std::vector<double>, std::vector<double>> dijkstra_star_planning(
    vector<double>  sx,
    vector<double>  sy,
    vector<double>  gx,
    vector<double>  gy,
    ...
    double rr)
{
    ...
    while (true)
    {
        ...
        // every 10 iteration, plot the current path
        if (pq.size() % 10 == 0)
        {
            //clear the canvas
            plt::clf();
...
int main()
{
    ...
    // set the fix size of the canvas and the axis
    ...
    std::vector<double> rx;
    std::vector<double> ry;
    std::tie(rx, ry) = dijkstra_star_planning(sx, sy, gx, gy, ox, oy, grid_size, robot_size);
    plt::plot(rx, ry, "-r");
    plt::pause(0.001);
    plt::show();
    return 0;
