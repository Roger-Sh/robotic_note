...
std::pair<std::vector<double>, std::vector<double>> a_star_planning(
    vector<double> sx,
    vector<double> sy,
    vector<double> gx,
    vector<double> gy,
...
int main()
{
    ...
    plt::plot(ox, oy, ".k");   // obstacle
    plt::plot(sx, sy, "og");   // start point
    plt::plot(gx, gy, "xb");   // goal point
    plt::grid(true);
    plt::axis("equal");

    std::vector<double> rx;
    std::vector<double> ry;
    std::tie(rx, ry) = a_star_planning(sx, sy, gx, gy, ox, oy, grid_size, robot_size);
    plt::plot(rx, ry, "-r");   // route
    plt::pause(0.001);
    plt::show();
    return 0;
}
