 ...
 * @brief Implementation of class NLinkArm3d
 * @version 0.1
 * @date 2023-05-31
 *
...
// Global paramters
const int INV_KINEMATIC_ITERATION(500);
...
public:
     ...
     */
    void get_points(
        const Eigen::VectorXd joint_angle_list,
        Eigen::VectorXd& x_list,
        Eigen::VectorXd& y_list,
        Eigen::VectorXd& z_list)
    {
        this->set_joint_angles(joint_angle_list);

        x_list.resize(this->link_list_.size() + 1);
        y_list.resize(this->link_list_.size() + 1);
        z_list.resize(this->link_list_.size() + 1);
        ...
        {
            trans = trans * this->link_list_[i].tranformation_matrix();
            x_list[i + 1] = trans(0, 3);
            y_list[i + 1] = trans(1, 3);
            z_list[i + 1] = trans(2, 3);
        }
    }
    ...
    /**
     * @brief draw arm with rrt path and obstacle
     *
     * @param obstacle_list
     * @param start_joints
     * @param end_joints
        ...
        bool pause = false)
    {
        ...
        matplot::ylabel("y");
        matplot::zlabel("z");
        // double limit = (this->link_list_.size() + 1) * 0.1;
        double limit = 1.5;
        ...
        // draw start_pose
        plot_arm_with_joints(start_joints, matplot::color::blue);

        // draw end_pose
        plot_arm_with_joints(end_joints, matplot::color::red);

        // draw path
        for (size_t i = 1; i < path.size() - 1; i++)
        {
            // draw path step
            plot_arm_with_joints(path[i], matplot::color::black);
    ...
    /**
     * @brief plot obstacle like a sphere
     *
     * @param obstacle_list
     */
    void plot_obstacle(std::vector<std::vector<double>>& obstacle_list)
    {
        for (size_t i = 0; i < obstacle_list.size(); i++)
        {
            ...
            std::vector<double> t = matplot::iota(0, M_PI / 500, M_PI);
            std::vector<double> xt = matplot::transform(
                t,
                [osize, ox](auto x)
                {
                    return ox + osize * sin(x) * cos(100 * x);
                });

            std::vector<double> yt = matplot::transform(
                t,
                [osize, oy](auto x)
                {
                    return oy + osize * sin(x) * sin(100 * x);
                });
            std::vector<double> zt = matplot::transform(
                t,
                [osize, oz](auto x)
                {
                    return oz + osize * cos(x);
                });
    ...
    /**
     * @brief plot arm with joint angles
     *
     * @param joints
     * @param color
     */
    void plot_arm_with_joints(Eigen::VectorXd& joints, matplot::color color)
    {
        // update arm joints
        this->set_joint_angles(joints);
        ...
        fig->line_width(3);
        fig->color(color);
        fig->marker_indices({x_list.size() - 1}).marker_size(10).marker_style(".");
    }
};
